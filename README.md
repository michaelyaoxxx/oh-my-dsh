...
- 主仓默认分支 `main`；harness 稳定分支 `master`，dsh-web 稳定分支 `main`（其默认分支是 `dev`，不要 pin `dev`）。
...
| `plugins/` | 插件 submodule 集合（dsh-web pin `main`、dsh-better-sidebar pin tag `v0.18.1`、dsh-plugin-mineru pin `master`、modlens pin tag `v3.26.1`），插件默认安装位置 |
