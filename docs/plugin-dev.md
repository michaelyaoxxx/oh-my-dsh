...
- 跑完 `make setup` 插件 submodule 变脏（如 dsh-automation 的 `lib/client.js`）：本地重建产物与 pin 自带的产物必然不同（CSS module 类名哈希由绝对源码路径派生）。入口文件已提交在仓内的插件不该本地构建——这类组件在组件目录里标 `prepareMode: tracked-prebuilt`（`dsh-automation`、`dsh-at-file`），`setup.sh` / `remote-install.sh` 经共用 executor 跳过构建；判据**只有** `prepareMode` 这一处（不再是「根 `main` 被跟踪」那条已删除的启发式，见上面 checklist 第 3 条的历史说明）。误改后 `git -C plugins/<name> checkout -- <file>` 还原。
