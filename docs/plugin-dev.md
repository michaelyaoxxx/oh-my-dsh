...
- 跑完 `make setup` 插件 submodule 变脏（如 dsh-automation 的 `lib/client.js`）：本地重建产物与 pin 自带的产物必然不同（CSS module 类名哈希由绝对源码路径派生）。入口文件已提交在仓内的插件不该本地构建——`setup.sh`/`remote-install.sh` 已按「根 `main` 被 git 跟踪即跳过构建」处理；误改后 `git -C plugins/<name> checkout -- <file>` 还原。
