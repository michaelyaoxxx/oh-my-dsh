...
> 见 [deployment.md](cicd_deployment.md)。两者的关系：本节说「谁负责什么」，那份说「跑在哪台机器上」。
...
**上图的物理落点**（详见 [deployment.md](cicd_deployment.md) §1）：
...
**现状是最大的一块空白**：本仓 CI 只做 pin 校验 + shellcheck + `make setup` + 冒烟，
**不跑任何 submodule 自己的测试**——而 harness 有 962 个测试文件、10 个插件合计 700+ 个。

完整方案（分层 L0-L4、submodule 修改策略、组合层验收）已独立成文：[testing.md](testing.md)。

要点三条：
1. **L1（各仓自测）是最大缺口**，且实测「一跑就有红的」（见 [testing.md](testing.md) §1.3）。
2. **L3（组合层验收）是本仓特有且完全缺失的**——各仓自测全绿 ≠ 组合可用，
   本仓已有三个实例证明（patch 抹旁键 / 同插件双挂 / LAN bind 冲突）。
3. **「是否要改 submodule」是个前置决策**，会连带改 `.gitmodules`、pin 语义与上游同步流程。
