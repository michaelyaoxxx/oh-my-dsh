...
| B14 | **密钥当前明文落盘（静态非明文方案未落地）** | 2026-09-15 盘点：DSH 自有凭据 `.dsh/.credentials.yaml`（0600）与已登记插件独立仓 `~/.modsearch/config.json`、`~/.modlens/config.json`（0600）均可含**明文**密钥；环境变量注入是唯一「不落盘」路径。**版本库侧已被硬约束**：`save-settings.mjs` 的 `secretPolicy`（env-ref / none）拒绝把字面密钥写进基线，`make save-settings` 对含字面密钥的 live 会拒绝导出——凡 `~/.modsearch` 等含 key 时无法进版本库，这正是消除泄露面的有意设计。**过渡期约定**：明文仅存在于 **0600** 的 live 文件与环境变量，脚本/仓库/日志里一律零密钥 | 按 [docs/secret-storage-design.md](secret-storage-design.md) 落地「静态非明文」：首选 systemd `EnvironmentFile=`(0600)/`LoadCredential=` 注入（服务器零代码改动）；密钥需进 git 时走 sops(+age)，主密钥（age 私钥）不进仓库 |
