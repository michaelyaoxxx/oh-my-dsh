...
- 子仓（submodule，固定 commit）：
  ...
  - `plugins/dsh-automation/` ← [titanwings/dsh-automation](https://github.com/titanwings/dsh-automation)，正式 tag `v0.1.7`（tag pin）
...
│   └── dsh-automation/        # [submodule] titanwings/dsh-automation，pin tag v0.1.7
...
1. `release.sh` 校验：工作区干净；分支-pin 子仓（dsh-web→`main`、dsh-plugin-mineru→`master`）的 pin 与远端对应分支上真实存在的 commit 一致（拦截"本地未推送的 commit 被误 pin"）；tag-pin 子仓（harness→`dsh-v0.1.5-rc.2`、dsh-better-sidebar→`v0.18.1`、modlens→`v3.26.1`、dsh-automation→`v0.1.7`）以远端正式 tag 比对（tag 存在于远端即已发布，同语义）。
