...
- **稳定分支**：harness → `master`；dsh-web → `main`（注意 dsh-web 默认分支是 `dev`，不要 pin `dev`）；dsh-better-sidebar → 正式 tag `v0.18.1`（tag pin；verify.yaml 与 release.sh 以 tag 比对校验）；dsh-plugin-mineru → `master`；modlens → 正式 tag `v3.26.1`（tag pin）。
